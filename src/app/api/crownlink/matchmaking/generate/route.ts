...
export async function POST(
  ...
  try {
     ...
     */
    let createdMatches:
      Array<GeneratedMatchInsert & { id: string }> =
        [];
    ...
    if (
      ...
      createdMatches =
        (data ??
          []) as Array<
          GeneratedMatchInsert & { id: string }
        >;

      /*
       * Send Discord matchup notifications
       * after the approved matches are safely
       * stored. A Discord failure must never
       * roll back or block matchmaking.
       */
      const notificationResults =
        await Promise.all(
          createdMatches.map((match) =>
            sendMatchApprovedDiscordNotifications(
              match.id
            )
          )
        );

      const discordNotificationsSent =
      ...
      const discordNotificationsSkipped =
        notificationResults.reduce(
          (total, current) =>
      ...
      const discordNotificationsFailed =
        notificationResults.reduce(
          (total, current) =>
            total + current.failed,
          0
        );
